...
6.  **`src/04_calculate_distance_rasters.R`**: **Создание растров расстояний до объектов OSM.**
    ...
    * **Обоснование выбора:** Финальная версия использует высокопроизводительный гибридный подход: быстрая растеризация вектора через `sf::gdal_utils("rasterize")` и последующий расчет расстояний на растре средствами `terra::distance()`. Этот метод оказался самым быстрым и надежным, не зависящим от системных настроек GDAL.

7.  **`src/05_generate_lagged_rasters.R`**: **Создание растров с временными лагами.**
    * **Что делает:** Рассчитывает накопленные осадки и среднюю температуру за предыдущие 3 и 6 месяцев для каждого месяца в исследуемом периоде.
    * **Обоснование выбора:** Лаговые переменные критически важны для моделирования, так как они отражают предшествующие условия (например, засушливость), которые могут влиять на возникновение пожаров сильнее, чем погода в конкретный день.

8.  **`src/06_create_ml_dataset.R`**: **Создание итогового датасета для ML.**
    * **Что делает:** Собирает все подготовленные данные в единую аналитическую таблицу. Генерирует случайные точки в каждом регионе и для каждой точки извлекает значения из всех статических, динамических и лаговых растров за каждый месяц, создавая финальный датасет.
    * **Обоснование выбора:** Такой "длинный" формат данных является оптимальным для построения пространственно-временных моделей. Скрипт также выполняет финальную очистку данных и инжиниринг признаков (например, создание циклических переменных для времени).

---
### Этап 2: Моделирование и проведение экспериментов

На этом этапе мы переходим от подготовки данных к их анализу и построению регрессионных моделей. Вся работа сосредоточена в едином, мощном скрипте-конвейере.
...
9.  **`src/07_master_modeling_pipeline.R`**: **Главный конвейер для проведения экспериментов.**
    ...
    * **Обоснование и ключевые особенности:** Скрипт спроектирован с упором на гибкость и воспроизводимость:
        * **"Пульт управления" (`CONFIG`):** Все параметры эксперимента (фильтры, наборы предикторов, гиперпараметры моделей) вынесены в единый конфигурационный блок. Это позволяет легко настраивать и запускать новые эксперименты, не меняя код самого конвейера.
        ...
        * **Воспроизводимость:** Для каждого запуска автоматически создается уникальная папка, название которой отражает все ключевые параметры эксперимента (`[Регион]_[Кол-во_строк]_[Фильтры]_[Предикторы]_[Гиперпараметры]`). Внутри сохраняется полный лог конфигурации, что гарантирует 100% воспроизводимость результатов.
        * **Комплексная оценка:** Конвейер автоматически обучает выбранный набор моделей, рассчитывает стандартные метрики качества (R-squared, RMSE и др.), сохраняет их в сравнительную таблицу и строит графики важности признаков для интерпретируемых моделей.

**(Здесь будут добавлены описания пакетных запусков)**
